...
int queueStatService_getServiceStatVal(service_statistics_type *serviceStatistics, double* statVal){
	...
	if (serviceStatistics != NULL && serviceStatistics->qService!=NULL) {
		pthread_mutex_lock(&serviceStatistics->qService->sampleQueue->lock);
		double qs = ((double)arrayList_size(serviceStatistics->qService->sampleQueue->queue));
		pthread_mutex_unlock(&serviceStatistics->qService->sampleQueue->lock);
		double mqs = ((double)serviceStatistics->qService->sampleQueue->max_queue_size);
		*statVal=(double)((qs/mqs)*100.0f);
