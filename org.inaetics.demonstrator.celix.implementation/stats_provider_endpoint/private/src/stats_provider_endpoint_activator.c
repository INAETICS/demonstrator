...
celix_status_t bundleActivator_start(void * userData, bundle_context_pt context) {
	...
	if (endpointService) {
		...
		endpointService->handleRequest = statsProviderEndpoint_handleRequest;
		endpointService->setService = statsProviderEndpoint_setService;

		properties_pt props = properties_create();
		properties_set(props, (char *) "remote.interface", (char *) INAETICS_DEMONSTRATOR_API__STATS_PROVIDER_SERVICE_NAME);
...
celix_status_t bundleActivator_stop(void * userData, bundle_context_pt context) {
	...
	if (activator->endpointService) {
		serviceStatisticsEndpoint_destroy(&activator->endpointService->endpoint);
